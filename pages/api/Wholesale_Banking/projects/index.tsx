...
/** 
 * @swagger
 *  /api/Wholesale_Banking/projects:
 *    post:
 *      summary: Insert a Projet object under Wholesale_Banking to MongoDB database.
 *      description: this api is used to insert a Projet object under Wholesale_Banking to MongoDB database
 *      requestBody:
 *        content:
 *          application/json:
 *            schema:
...
async function ProjectHandler(req: NextApiRequest, res: NextApiResponse<{}>) {
    if(req.method=='POST'){
       ...
       const db=client.db()
       const result=await db.collection('Wholesale_banking').insertOne(project)
       client.close()
 ...
 *  /api/Wholesale_Banking/projects:
 *    get:
 *      summary: Get the details of a project under Wholesale_Banking by its name.
 *      description: Get the details of a project under Wholesale_Banking by its name from MongoDB database.
 *      parameters:
 *        - in: query
 *          name: project_name
 *          schema:
 ...
 *  /api/Wholesale_Banking/projects:
 *    delete:
 *      summary: Delete a project under Wholesale_Banking by its name.
 *      description: Delete a project under Wholesale_Banking by its name from MongoDB database.
 *      parameters:
 *        - in: query
 *          name: project_name
 *          schema:
...
*/
      ...
      const client= await MongoClient.connect('mongodb+srv://sdcode001:j92txQCN7mUJWGmx@cluster0.3yr72o9.mongodb.net/BankDatabase?retryWrites=true&w=majority')
      const db=client.db()
      const result=await db.collection('Wholesale_banking').deleteOne({name:project_name})
      client.close()
